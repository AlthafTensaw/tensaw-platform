...
export function AppealTab({ caseId }: AppealTabProps): React.ReactElement {
  ...
  if (appeal === undefined || appeal === null) {
    return <GenerateView caseId={caseId} />;
  }

  return <EditView appeal={appeal} caseId={caseId} />;
}
...
function GenerateView({ caseId }: GenerateViewProps): React.ReactElement {
  const [fireGenerate, { isPending, error }] = useActionMutation('case.appeal.generate');
...
function EditView({ appeal, caseId }: EditViewProps): React.ReactElement {
  const [fireSave, { isPending: savePending, error: saveError }] = useActionMutation('case.appeal.save');
  const [fireGenerate, { isPending: regenPending, error: regenError }] = useActionMutation('case.appeal.generate');
