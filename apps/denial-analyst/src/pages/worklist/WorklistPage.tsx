...
export function WorklistPage() {
  ...
  const { data, isLoading, refetch } = useActionQuery<WorklistResponse>('denial.list', { ...filters, page });
  ...
  return (
    <DataExplorer<WorklistRow>
      ...
      totalRows={total}
      getRowId={(r: WorklistRow) => r.classification.classification_id}
      loading={isLoading}
      empty={{
      ...
      pageIndex={page - 1}
      pageSize={pageSize}
      onPageChange={(next: number) => setPage(next + 1)}
      // Selection
      selectionMode="multi"
      ...
      renderRowDetail={(row: WorklistRow) => (
