...
export function WorkflowStepsList({
  ...
  const handleCheck = (step: WorkflowStep) => {
    ...
    fireStep({
      ...
      .then((resp) => {
        onStepCompleted(resp as any); // Ignoring the exact type mismatch for onStepCompleted for now, we just need to fix auto_completed_classification
        if (resp.ok && resp.data.auto_completed_classification) onAutoComplete();
      })
      .catch(() => {
