...
import type { CategoriesResponse, LookupResponse } from '../../actions/schemas-v4-tabs';
...
export function WorklistFiltersBar(): React.ReactElement {
  ...
  const { data: categoryData } = useActionQuery<CategoriesResponse>('category.list', {});
  const { data: clinicData } = useActionQuery<LookupResponse>('lookup.clinics', {});
  const { data: payerData } = useActionQuery<LookupResponse>(
    'lookup.payers',
