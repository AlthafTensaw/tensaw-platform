...
import { DenialHistoryStrip } from './DenialHistoryStrip';
...
export function RowDetailPanel({ row, onMutated }: RowDetailPanelProps) {
  ...
  const { data: claimDetail, isLoading: detailLoading } = useActionQuery<ClaimDetail>('denial.claim-detail', claimDetailRequest);

  const { data: eventsData, isLoading: eventsLoading } = useActionQuery<DenialEvent[]>('denial.denial-events', denialEventsRequest);
  ...
  return (
    <div className="bg-muted/50">
      ...
      {/* v2.2: Denial history strip — self-hides when count < 2 */}
      <div className="px-4">
        <DenialHistoryStrip
