...
import type { FilesResponse, FileEntity, FileType } from '../../actions/schemas-v4-tabs';
...
export function FilesTab({ case: c }: FilesTabProps): React.ReactElement {
  const { data, isLoading, error, refetch } = useActionQuery<FilesResponse>(
    'case.files',
    { case_id: c.case_id },
  );
      ...
      {ORDER.filter((t) => groups[t] !== undefined && groups[t].length > 0).map((type) => {
        ...
        return (
          <section key={type} aria-labelledby={`group-${type}`}>
            ...
            >
              {fileTypeLabel(type)} ({group.length})
            </h3>
...
function UploadPicker({ caseId }: UploadPickerProps): React.ReactElement {
  const [fireUpload, { isLoading: isPending, error }] = useActionMutation('case.file.upload');
