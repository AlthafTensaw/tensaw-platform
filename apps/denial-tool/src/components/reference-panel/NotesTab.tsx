...
import type { NotesResponse, Note, NoteSource } from '../../actions/schemas-v4-tabs';
...
export function NotesTab({ case: c }: NotesTabProps): React.ReactElement {
  const { data, isLoading, error, refetch } = useActionQuery<NotesResponse>(
    'case.notes',
    { case_id: c.case_id },
  );
  const [fireAdd, { isLoading: addPending, error: addError }] = useActionMutation('case.note.add');
