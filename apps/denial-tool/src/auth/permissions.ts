...
// PR-8: hook used by components to check permissions inline.
// Reads roles from useAuthStore and resolves via resolvePermissions().
...
export function usePermissions(): {
  ...
  const user = useAuthStore((s: { user: { roles?: string[] } | null }) => s.user);
  const roles = (user?.roles ?? []).filter((r: string): r is Role =>
