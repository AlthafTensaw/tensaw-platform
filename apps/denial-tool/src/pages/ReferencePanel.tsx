...
import type { NotesResponse, FilesResponse, Appeal } from '../actions/schemas-v4-tabs';
...
function ReferencePanelLoaded({ caseId }: ReferencePanelLoadedProps): React.ReactElement {
  ...
  const { data, isLoading, error } = useActionQuery<CaseDetail>(
    'case.detail',
    { case_id: caseId },
  );
  ...
  const { data: notesData } = useActionQuery<NotesResponse>('case.notes', { case_id: caseId });
  const { data: filesData } = useActionQuery<FilesResponse>('case.files', { case_id: caseId });
  const { data: appealData } = useActionQuery<Appeal | null>('case.appeal.get', { case_id: caseId });
