...
import type { WorklistResponse } from '../actions/schemas-v4';
...
export function WorklistPane(): React.ReactElement {
  ...
  const {
    ...
    error,
    refetch,
  } = useActionQuery<WorklistResponse>('case.worklist', queryParams ?? {}, {
    skip: queryParams === null,
  });

  // Lookup the queue label for empty-state messaging
  const { data: queueListData } = useActionQuery<{ queues: Array<{ queue_id: string; queue_label: string }> }>('queue.list', {});
