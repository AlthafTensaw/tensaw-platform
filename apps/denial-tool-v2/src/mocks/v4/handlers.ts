...
function jsonError(message: string, status = 400): HttpResponse<{ error: string }> {
...
export const handlersV4 = [

  // ──────────────────────────────────────────────────────────────────
  // case.worklist
  // ──────────────────────────────────────────────────────────────────
  http.get('*/api/v1/cases/worklist', ({ request }) => {
  ...
  // case.tasks
  // ──────────────────────────────────────────────────────────────────
  http.get('*/api/v1/cases/:caseId/tasks', ({ params }) => {
    const caseId = params.caseId as string;
    const c = getCase(caseId);
    if (!c) return jsonError('case not found', 404);
    return HttpResponse.json({
  ...
  // task.mine
  // ──────────────────────────────────────────────────────────────────
  http.get('*/api/v1/tasks/mine', ({ request }) => {
    const url = new URL(request.url);
    const params = Object.fromEntries(url.searchParams);
  ...
  http.get('*/api/v1/cases/:caseId/appeal/:appealId', ({ params }) => {
    const appeal = getAppeal(params.appealId as string);
  ...
  http.get('*/api/v1/lookups/clinics/:clinicId/providers', ({ params }) => {
    return HttpResponse.json({
      items: PROVIDERS_BY_CLINIC[params.clinicId as string] ?? [],
    });
  }),

  http.get('*/api/v1/lookups/clinics/:clinicId/payers', ({ params }) => {
    return HttpResponse.json({
      items: PAYERS_BY_CLINIC[params.clinicId as string] ?? [],
    });
  }),

  http.get('*/api/v1/lookups/clinics/:clinicId/facilities', ({ params }) => {
    return HttpResponse.json({
  ...
  // case.accept
  // ──────────────────────────────────────────────────────────────────
  http.post('*/api/v1/cases/:caseId/accept', async ({ request, params }) => {
    const caseId = params.caseId as string;
  ...
  // case.override
  // ──────────────────────────────────────────────────────────────────
  http.post('*/api/v1/cases/:caseId/override', async ({ request, params }) => {
    const caseId = params.caseId as string;
  ...
  // case.reclassify
  // ──────────────────────────────────────────────────────────────────
  http.post('*/api/v1/cases/:caseId/classify', ({ params }) => {
    const caseId = params.caseId as string;
  ...
  // ──────────────────────────────────────────────────────────────────
  http.post(
    '*/api/v1/cases/:caseId/tasks/:taskId/complete',
  ...
  // case.signal
  // ──────────────────────────────────────────────────────────────────
  http.post('*/api/v1/cases/:caseId/signal', async ({ request, params }) => {
    const caseId = params.caseId as string;
    const c = getCase(caseId);
    if (!c) return jsonError('case not found', 404);
    const body = (await request.json().catch(() => null)) as {
  ...
  // ──────────────────────────────────────────────────────────────────
  http.post(
    '*/api/v1/cases/:caseId/appeal/generate',
    async ({ request, params }) => {
      const caseId = params.caseId as string;
      const c = getCase(caseId);
      if (!c) return jsonError('case not found', 404);
...
Primrose RCM`;
  ...
  // ──────────────────────────────────────────────────────────────────
  http.put(
    '*/api/v1/cases/:caseId/appeal/:appealId',
    async ({ request, params }) => {
      const body = (await request.json().catch(() => null)) as {
        body?: string;
  ...
  // ──────────────────────────────────────────────────────────────────
  http.post('*/api/v1/cases/:caseId/reveal-phi', async ({ request, params }) => {
    const caseId = params.caseId as string;
    const c = getCase(caseId);
    if (!c) return jsonError('case not found', 404);
    const body = (await request.json().catch(() => null)) as {
