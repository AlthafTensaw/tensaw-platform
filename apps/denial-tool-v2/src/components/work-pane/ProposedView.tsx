...
export function ProposedView({
  ...
  const { data: catData } = useActionQuery('category.list', {});
  const [fireAccept, { isPending: acceptPending, error: acceptError }] =
    useActionMutation('case.accept');
  const [fireReclassify, { isPending: reclassifyPending }] =
